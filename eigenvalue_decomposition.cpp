...
Matrix parseMatrix(const string& input) {
    ...
    while (getline(ss, row, ';')) {
        ...
        while (getline(rs, val, ',')) {
            if (!val.empty()) r.push_back(stod(val));
        }
...
int main() {
    ...
    Matrix A = parseMatrix(matrixStr);
    int n = size(A);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        A = subtract(A, outerProd);  // deflate
    }

    Matrix P_inv = inverse(P);
    Matrix A_reconstructed = multiply(multiply(P, D), P_inv);
